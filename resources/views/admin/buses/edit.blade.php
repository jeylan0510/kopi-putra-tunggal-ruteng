...
@section('content')
    <div class="mb-6 flex items-center gap-4">
        ...
        <div>
            <h2 class="text-2xl font-bold text-slate-800">Edit Bus</h2>
            <p class="text-slate-500 text-sm mt-1">Perbarui detail armada bus.</p>
        </div>
    </div>
        ...
        <form action="{{ route('admin.buses.update', $bus->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <input type="text" name="nama_bus" value="{{ old('nama_bus', $bus->nama_bus) }}" class="w-full border-slate-200 rounded-xl focus:ring-cyan-500 focus:border-cyan-500" required>
                ...
                <div>
                    ...
                    <input type="text" name="nomor_polisi" value="{{ old('nomor_polisi', $bus->nomor_polisi) }}" class="w-full border-slate-200 rounded-xl focus:ring-cyan-500 focus:border-cyan-500" required>
                ...
                <div>
                    ...
                    <input type="number" name="kapasitas" value="{{ old('kapasitas', $bus->kapasitas) }}" class="w-full border-slate-200 rounded-xl focus:ring-cyan-500 focus:border-cyan-500" required min="1">
                ...
                <div>
                    ...
                    <select name="kelas" class="w-full border-slate-200 rounded-xl focus:ring-cyan-500 focus:border-cyan-500" required>
                        <option value="Ekonomi" {{ old('kelas', $bus->kelas) == 'Ekonomi' ? 'selected' : '' }}>Ekonomi</option>
                        <option value="Bisnis" {{ old('kelas', $bus->kelas) == 'Bisnis' ? 'selected' : '' }}>Bisnis</option>
                        <option value="Executive" {{ old('kelas', $bus->kelas) == 'Executive' ? 'selected' : '' }}>Executive</option>
                    </select>
                ...
                <div>
                    ...
                    <select name="status" class="w-full border-slate-200 rounded-xl focus:ring-cyan-500 focus:border-cyan-500" required>
                        ...
                        <option value="Tidak Aktif" {{ old('status', $bus->status) == 'Tidak Aktif' ? 'selected' : '' }}>Tidak Aktif</option>
                    </select>
                ...
                <div class="md:col-span-2">
                    ...
                    @if($bus->gambar)
                        <div class="mb-3">
                            <img src="{{ Storage::url($bus->gambar) }}" alt="{{ $bus->nama_bus }}" class="w-48 h-32 object-cover rounded-xl shadow-sm">
                        </div>
                    @endif
