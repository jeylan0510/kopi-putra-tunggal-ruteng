...
@section('content')
    <div class="mb-6 flex items-center gap-4">
        ...
        <div>
            <h2 class="text-2xl font-bold text-slate-800">Tambah Jadwal Baru</h2>
            <p class="text-slate-500 text-sm mt-1">Atur jadwal keberangkatan bus dan rute.</p>
        </div>
    </div>
        ...
        <form action="{{ route('admin.schedules.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <select name="route_id" class="w-full border-slate-200 rounded-xl focus:ring-cyan-500 focus:border-cyan-500" required>
                        ...
                        @foreach($routes as $route)
                            <option value="{{ $route->id }}" data-estimasi="{{ $route->estimasi }}" {{ old('route_id') == $route->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="bus_id" class="w-full border-slate-200 rounded-xl focus:ring-cyan-500 focus:border-cyan-500" required>
                        ...
                        @foreach($buses as $bus)
                            <option value="{{ $bus->id }}" data-kapasitas="{{ $bus->kapasitas }}" {{ old('bus_id') == $bus->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="date" name="tanggal" value="{{ old('tanggal') }}" class="w-full border-slate-200 rounded-xl focus:ring-cyan-500 focus:border-cyan-500" required>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="time" name="jam_berangkat" id="jam_berangkat" value="{{ old('jam_berangkat') }}" class="w-full border-slate-200 rounded-xl focus:ring-cyan-500 focus:border-cyan-500" required>
                    ...
                    <div>
                        ...
                        <input type="time" name="jam_tiba" id="jam_tiba" value="{{ old('jam_tiba') }}" class="w-full border-slate-200 rounded-xl focus:ring-cyan-500 focus:border-cyan-500" required>
                ...
                <div>
                    ...
                    <input type="number" name="harga" value="{{ old('harga') }}" class="w-full border-slate-200 rounded-xl focus:ring-cyan-500 focus:border-cyan-500" placeholder="Contoh: 150000" required min="0">
                ...
                <div>
                    ...
                    <input type="number" name="kursi_tersedia" id="kursi_tersedia" value="{{ old('kursi_tersedia') }}" class="w-full border-slate-200 rounded-xl bg-slate-50 text-slate-500 focus:ring-cyan-500 focus:border-cyan-500 cursor-not-allowed" placeholder="Otomatis terisi sesuai bus" readonly>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Trigger change on load if already selected (e.g. from old input)
            if(busSelect.value) {
